...
# Start the pipeline and ensure we always stop it (safe cleanup)
try:
    ...
    for i in range(NUM_FRAMES):
        ...
        # get depth and color frames; color will be saved to the bag because the stream is enabled
        depth_frame = frames.get_depth_frame()
        color_frame = frames.get_color_frame()

        if not depth_frame and not color_frame:
            # neither frame available, skip
            continue

        # process depth frame with filters as before
        if depth_frame:
        ...
        # we don't apply filters to the color frame here; accessing it is enough so it's part of the recording
        if color_frame:
            # simple access to color frame to ensure it's retrieved - no processing required now
            _ = color_frame.get_data()
